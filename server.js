...
async function connectToMongo() {
  try {
    ...
    // --- NEW: Setup for Group Chats ---
    groupsCollection = db.collection("groups");
    groupMessagesCollection = db.collection("groupMessages"); // For offline relay

    // 1. Index for finding groups a user is in
    await groupsCollection.createIndex({ "members.pubKey": 1 });

    // 2. TTL Index for 14-day expiry on group messages
    await groupMessagesCollection.createIndex({ "expireAt": 1 }, { expireAfterSeconds: 0 });

    // 3. Index for finding all messages for a specific group
    await groupMessagesCollection.createIndex({ groupId: 1 });

    // 4. Index for finding all pending messages for a specific recipient
    await groupMessagesCollection.createIndex({ "pendingRecipients.pubKey": 1 });

    // 5. Index for finding all messages sent by a specific user (for their Relay Manager)
    await groupMessagesCollection.createIndex({ senderPubKey: 1 });

    console.log("✅ Group chat collections and indexes are ready.");
    // --- END NEW ---
...
// --- END NEW ---
  ...
  // --- NEW: Check for offline GROUP messages ---
    (async () => {
      try {
        const messages = await groupMessagesCollection.find({ "pendingRecipients.pubKey": key }).toArray();
        if (messages.length > 0) {
          console.log(`[Group] Found ${messages.length} offline group messages for ${key.slice(0,6)}...`);
        ...
        console.error(`[Group] Failed to send offline group messages to ${key.slice(0,6)}: ${e}`);
      }
    })();
    // --- END NEW ---
...
// -- Video/Voice Call Signaling --
  ...
  /**
   * (Phase 3A) Admin creates the group and registers it with the server.
   */
  socket.on("group:create", async (data) => {
    const { groupId, groupName, members } = data; // members is [{pubKey, senderKey}, ...]
    const adminPubKey = socket.data.pubKey;
    if (!adminPubKey) return; // Not registered
    ...
    try {
      await groupsCollection.insertOne({
        ...
        name: groupName,
        admin: adminPubKey,
        members: members, // Store the initial list of members and their sender keys
        createdAt: new Date()
      });
      console.log(`[Group] Admin ${adminPubKey.slice(0,6)} created group ${groupName} (${groupId})`);
    } catch (e) {
      console.error(`[Group] Failed to create group ${groupId}: ${e}`);
      // You could emit an error back to the admin here
    }
  });
  ...
  /**
   * (Phase 3B) The main message relay logic (Your "Distributed Gold" model)
   */
  socket.on("group:message", async (data) => {
    const { groupId, encryptedMessageBlob } = data;
    const senderPubKey = socket.data.pubKey;
    if (!senderPubKey || !groupId || !encryptedMessageBlob) return;

    try {
      const group = await groupsCollection.findOne({ _id: groupId });
      if (!group) return; // Not a valid group

      // Verify sender is a member
      if (!group.members.some(m => m.pubKey === senderPubKey)) return;

      // Prepare the payload to be sent/stored
      const payload = {
        groupId: groupId,
        senderPubKey: senderPubKey,
        blob: encryptedMessageBlob
        // We'll add 'messageId' later from the DB
      };

      // This is the "Distributed Gold" quota check, just like your 1-on-1 relay
      const payloadSizeBytes = Buffer.from(encryptedMessageBlob, 'base64').length;

      // --- BEGIN QUOTA CHECK ---
      // NOTE: This uses the *groupMessagesCollection* to calculate quota
      const userMessages = await groupMessagesCollection.find({ senderPubKey }).toArray();
      let currentGroupUsage = 0;
      userMessages.forEach(msg => {
        // Check how many recipients are still pending for this message
        const pendingCount = msg.pendingRecipients ? msg.pendingRecipients.length : 0;
        currentGroupUsage += (msg.sizeBytes || 0) * pendingCount;
      });

      // We'll give 0.5MB for group quota, as you suggested
      const GROUP_QUOTA_BYTES = 512 * 1024; // 0.5MB
      if (currentGroupUsage + payloadSizeBytes > GROUP_QUOTA_BYTES) {
        console.warn(`[Group] Quota exceeded for ${senderPubKey.slice(0,6)}`);
        socket.emit("group:error", { groupId, error: "Group message quota exceeded." });
        return;
      }
      // --- END QUOTA CHECK ---
      ...
      group.members.forEach(member => {
        if (member.pubKey === senderPubKey) return; // Don't send to self

        const targetSocketId = userSockets[member.pubKey];
        if (targetSocketId) {
          // Member is ONLINE
          io.to(targetSocketId).emit("group:message_in", payload);
        } else {
          // Member is OFFLINE
          offlineRecipients.push({ pubKey: member.pubKey, senderKey: member.senderKey });
        }
      });
      ...
      if (offlineRecipients.length > 0) {
        const messageDoc = {
          ...
          sizeBytes: payloadSizeBytes,
          pendingRecipients: offlineRecipients, // List of {pubKey, senderKey}
          createdAt: new Date(),
          expireAt: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000) // 14-day TTL
        };
        const result = await groupMessagesCollection.insertOne(messageDoc);

        // Now that we have the ID, update the payload for online users
        payload.messageId = result.insertedId.toString(); 
        // We can re-emit to online users so they get the ID, but it's not critical
      }

      console.log(`[Group] Relayed message from ${senderPubKey.slice(0,6)} in ${groupId} (Online: ${group.members.length - offlineRecipients.length - 1}, Offline: ${offlineRecipients.length})`);

    } catch (e) {
      console.error(`[Group] Failed to relay message for ${groupId}: ${e}`);
    }
  });
